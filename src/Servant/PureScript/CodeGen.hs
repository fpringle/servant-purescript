...
genBuildQueryArgs args =
    "let queryArgs = catMaybes ["
        </> indent 2 (docIntercalate ("," <> softline) . map genBuildQueryArg $ args)
        </> "]"
...
genBuildHeader (HeaderArg arg) =
    let
     ...
     in
        align $
            "{ field : " <> dquotes encodedArgName
                <+/> comma
                    <+> "value :"
                    <+> "encodeHeader spOpts_'"
                    <+> psVar argText
                </> "}"
